...
class ORExplainer(nn.Module):
    def __init__(self, model, epochs: int = 20, lr: float = 0.003,
        ...
        # Explanation model in CExplainer
        in_channels = (x_dim[args.dataset] + args.hidden * self.num_hops) * 3
    ...
    def get_model_output(self, x, edge_index, edge_mask=None, **kwargs):
        ...
        with torch.no_grad():
            ...
            embeddings = self.model.embed(data.x, data.edge_index, data.edge_weight)
            logits = self.model.lin(embeddings[-1])
    ...
    def train_NC_explanation_network(self, data, energy):
        ...
        with torch.no_grad():
            ...
            for gid in tqdm(dataset_indices):
                ...
                embedding_dic[gid] = torch.cat([x, embeddings], dim=-1).cpu()
            data.detach()
